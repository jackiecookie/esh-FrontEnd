    ...
    MemberFrom = {
        ...
        addFormEvent: function (eventItem) {
            ...
            if (isCheck == 0) {
                alertify.confirm("一旦保存并发布,将进入审核状态,该产品将无法修改,是否确认提交", function (e) {
                    if (!e) return false;
                    MemberFrom.submitform.call(self, $this, isCheck);
                });
            } else {
                MemberFrom.submitform.call(this, $this, isCheck);
            }
        },
        submitform: function ($this, isCheck) {
            var self = this;
            if (self.submiting == 1) {
                return false;
            };
                    ...
                    if (d.Success) {
                        ...
                        if (data.Info.FILEPATH) {
                            $("#extraInfoLoading").css({
                                visibility: 'hidden'
                            }).append('<li class="clearfix tsuc"><p class="file clearfix"><span class="fl" style="height:100px;" ><img src="/' + data.Info.FILEPATH + '" onload="ResizeImageByObj(this, 100, 100,imgonLoadCallBack)"></span>   <a href="javascript:;" action-type="cancel" onclick="editDel()">删除</a></p></li>');
                        }
                        if (data.Info.FABRICTYPE_NAME) {
                            var elm = $("#srcTypeDiv").find(".selectLt");
                            elm[0] ? elm.html(data.Info.FABRICTYPE_NAME) : $("#srcTypeDiv").MultiCssSelect.susses(function () {
                        ...
                        if (data.Info.IS_CHECK == 1 || data.Info.IS_CHECK == 0 || data.Info.IS_CHECK == 4) {
                            var msg = data.Info.IS_CHECK == 0 ? '已经进入审核状态的面料无法修改' : '已经审核通过的面料无法修改';
                            appndMsg(msg);
                            $('#btnUpload .ui-upload-btn').remove();
                            $('#savebtn').remove();
                            $('#submitAndPublish').remove();
                        } else {
                            if (data.Info.IS_CHECK == 2)
                                appndMsg('抱歉,您没审核通过的理由为:' + data.Info.REASON);
                            $('#savebtn').removeClass('hide');
                            $('#submitAndPublish').removeClass('hide');
                        }
                    }
                    else {
        ...
        addComponentRow: function (typeIndex, type, data, isNotAdd) {
            ...
            if (data) {
                ...
                jxHtml += ' <input  type="text"  readonly="readonly" value="' + data.JXCF + '" class="w_small w_length  mr-5" /> ';
                jxHtml += '</td>';
                ...
                jxHtml += '<input name="iptcf" type="hidden"  value="' + dataValue + '" /><span class="form-tip"></span>';
                jxHtml += '</td>';
                jxHtml += '</tr>';

                jxHtml += '<tr>';
                jxHtml += ' <td class="td1" align="right">粗细</td>';
                jxHtml += ' <td class="text1">  <input type="text" readonly="readonly" value="' + data.JSGG + '" class="w_small w_length  mr-5" /> </td>';
                jxHtml += '<td></td>';
                jxHtml += '</tr>';
                jxHtml += '</table><div>';
                $dd.append(jxHtml);
                ...
                jxHtml += ' <td class="td1" align="right">成分</td>';
                jxHtml += ' <td class="text1">';
                jxHtml += ' <input  type="text"  readonly="readonly" class="w_small w_length  mr-5" /> ';
                jxHtml += '</td>';
                ...
                jxHtml += '<tr>';
                jxHtml += ' <td class="td1" align="right">粗细</td>';
                jxHtml += ' <td class="text1">  <input type="text" readonly="readonly" class="w_small w_length  mr-5" /> </td>';
                jxHtml += '<td></td>';
                jxHtml += '</tr>';
                jxHtml += '</table><div>';
                $dd.append(jxHtml);
