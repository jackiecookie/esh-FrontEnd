define(function (require, exports, module) {
    ...
    var getdata = function () {
        ...
        var c = $('#country').val();
        $.ajaxjson('/UIData/ashx/GetRusAbordData.ashx', { 'cn': company, 'c': c }, function (data) {
            $('#lf .table_b').show();
            $('#load').hide();
            if (data) {
                ...
                var a = ['#GY1', '#GY2', '#GY3'];
                var a1 = ['#CG1', '#CG2', '#CG3'];
                drawTable(a1, data.ProductRankResult.Year1, '/Report/RusAbroad?year=' + minYear + '&c=' + c + '&cn=' + encodeURIComponent(company) + '&pm=', 0, minYear);
                drawTable(a1, data.ProductRankResult.Year2, '/Report/RusAbroad?year=' + (parseInt(minYear) + parseInt(1)) + '&c=' + c + '&cn=' + encodeURIComponent(company) + '&pm=', 1, minYear + 1);
                drawTable(a1, data.ProductRankResult.Year3, '/Report/RusAbroad?year=' + (parseInt(minYear) + parseInt(2)) + '&c=' + c + '&cn=' + encodeURIComponent(company) + '&pm=', 2, minYear + 2);
                drawTable(a, data.ExportingCountryResult.Year1, '/Company/russiaDetail?cn=', 0, minYear);
                drawTable(a, data.ExportingCountryResult.Year2, '/Company/russiaDetail?cn=', 1, minYear + 1);
                drawTable(a, data.ExportingCountryResult.Year3, '/Company/russiaDetail?cn=', 2, minYear + 2);
