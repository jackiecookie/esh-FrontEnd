...
验证表单信息
*/
define(['css/common/Esh_validator/Esh_validator.css'],function(require, exports, module) {
    ...
    Fv.prototype = {
        ...
        /**
		 * 添加校验项
		 * @param ic:object	校验项配置信息(itemCfg)
		 * 
		 * @return this
		 */
        add: function (ic) {
            ...
            for (var i = 0, l = icList.length; i < l; i++) {
                ...
                item$.bind('focus blur', function (ic) {
                    return function (e) {
        ...
        remove: function (item) {
            ...
            for (var i = 0, l = itemList.length; i < l; i++) {
                ...
                if (this.ico[vid] && this.ico[vid].tip$) {
                    this.ico[vid].tip ? this.ico[vid].tip$.hide() : this.ico[vid].tip$.remove();
                    delete this.ico[vid];
                    this.count--;
                }
            }
        ...
        /**
		 * 修改校验项
		 * @param ic:object	单项配置信息(itemCfg)
		 * 
		 * @return this
		 */
        modify: function (ic) {
        ...
        /**
		 * 验证通过后回调
		 * @param fn:function				回调函数,回传参数校验结果[boolean]
		 * @param checkFailed2cback:boolean	设置为true,校验未通过仍执行回调
		 */
		 ...
		 * 单项检查
		 *  @param item:*		selector||[selector1, selector2, ...]
		 *  @param fn:function	Ajax校验完成回调[可选]
		 * 
		 * @return boolean|pending:string
		 */
        ...
        /**
		 * 全部检查
		 * @param hasPending:boolean	返回结果包含pending状态
		 * 
		 * @return boolean|pending:string
		 */
    ...
    function _tip(ic, result, ischeck) {
        ...
        if (!ic.require || hResult != result) {
            ...
            rmcls && tip$.removeClass(rmcls);
            tip$.addClass(addcls).text(ic.msg[result] || '');
            !ic.require && ischeck && result == 'normal' && tip$.hide();
