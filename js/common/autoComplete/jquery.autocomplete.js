...
*/
//自动补全功能
define(['css/common/autoComplete/jquery.autocomplete.css'], function () {
    (function ($) {
        $.fn.extend({
        ...
        var Controller = function (input, option) {
            if (option.getajaxParams) {
                option.ajaxParams = option.getajaxParams(input);
            }
            this.keywordQuere = {};
        ...
        var _ajaxSend = function (keyword) {
            ...
            ajaxOption = {
                ...
                'success': function (theData, textStatus, jqXHR) {
                    if (that.keywordQuere[keyword] != that.keywordQuere.length) {
                        return;
                    }
                    _loadDataDrawView.call(that, theData, keyword);
               //     data = theData;
                },
        ...
        var _search = function () {
            ...
            if (loadDataFlag) {
                if ($.isArray(this.option.data)) {
                    ...
                    try {
                        that.keywordQuere[keyword] = that.keywordQuere.length;
            ...
            //            }
            if (data)
                _loadDataDrawView.call(this, data, keyword);
        };

        var _loadDataDrawView = function (data, keyword) {
            var that = this,
            result = [];
            $.each(data, function (index, value) {
                if (that.option.maxItems > 0 && result.length >= that.option.maxItems)
