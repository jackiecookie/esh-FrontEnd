      ...
      if (obj) {
        if (obj.style) {
          ...
          stream.push(new File({
            ...
            path: path.join(filePath.dirname, obj.name,filePath.name + filePath.extname.toLowerCase()),
            contents: obj.style
          }));
        } else {
          //潘迪海修改：path增加一层路径 
          _.each(obj.sprites, function(sprite) {
            stream.push(new File({
              ...
              path: path.join(opt.out, '/' + sprite.name.split('-')[1], sprite.name + '.' + sprite.type.toLowerCase()),
